...
# --- CRITICAL FIX: WARM-UP THE MODEL ---
# TensorFlow compiles the graph on the first run. We do this now so it doesn't freeze live.
print("Warming up the AI brain... (This prevents the freeze later)")
dummy_input = np.zeros((1, max_length, 258))  # Create fake 'empty' data
model.predict(dummy_input, verbose=0)  # Force the first prediction now
...
def prob_viz(res, actions, input_frame, colors):
    ...
    for num, prob in enumerate(res):
        # ERROR PREVENTION: Check for NaN
